...
class Img{
	...
	static function creerMin($img,$chemin,$nom,$mlargeur=100,$mhauteur=100){
		...
		imagepng($miniature,$chemin."/".$nom.".png",9);
		return true;
	}

	static function creerMin2($img,$chemin,$nom,$mlargeur=100,$mhauteur=100){
		// On supprime l'extension du nom
		$nom = substr($nom,0,-4);
		// On récupère les dimensions de l'image
		$dimension=getimagesize($img);
	...
	static function convertirPNG($img){
		// On cré une image à partir du fichier récup
		if(substr(strtolower($img),-4)==".jpg"){$image = imagecreatefromjpeg($img); }
		else if(substr(strtolower($img),-4)==".png"){$image = imagecreatefrompng($img); }
		else if(substr(strtolower($img),-4)==".gif"){$image = imagecreatefromgif($img); }
		...
		unlink($img);
		imagepng($image,substr($img,0,-3)."png",9);	
		return true;
