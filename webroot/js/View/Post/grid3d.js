		...
		grid3D.prototype._showContent = function( pos ) {
			...
			var self = this,
				loadContent = function() {
					var contentDiv = $(self.contentItems[ pos ]);
					var postUrl = contentDiv.attr('data-url');
					var link = String(window.location.origin+postUrl);
			        link = $.trim(link);
			        $.ajax({
					  	method: "GET",
					  	url: link,
						...
						contentDiv.children(":first").html(response);
						contentDiv.children(":first").append('<span class="icon close-contentParent"><span class="icon close-content2"></span></span>');
						// hide loader
